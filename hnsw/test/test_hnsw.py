...
class TestHNSW(unittest.TestCase):
    ...
    def test_k_nearest_neighbors_search(self):
        ...
        for v in self.vectors:
            v_distance = np.dot(query_vector, v) / (np.linalg.norm(query_vector) * np.linalg.norm(v))
    ...
    def test_node_connections_limit(self):
        ...
        for layer, nodes in self.hnsw.layered_graph.items():
            for node in nodes:
                if layer == 0:
                    self.assertLessEqual(len(node.neighbors.get(layer, [])), self.hnsw.m0)
                else:
                    self.assertLessEqual(len(node.neighbors.get(layer, [])), self.hnsw.mMax)
